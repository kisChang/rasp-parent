...
public class RaspWiFiUtils {
    ...
    public static boolean connWiFi(String ssid, String key, String devName, String path) {
            String shell_connWifi = String.format(
                "#!/bin/sh\n" +
                "kill -9 $(ps aux | grep wpa_supplicant | grep -v grep | awk '{print $2}') || true \n" +
                "kill -9 $(ps aux | grep udhcpc | grep -v grep | awk '{print $2}') || true \n" +
                "ifconfig %s down\n" +
                "sleep 2s \n" +
                "ifconfig %s up\n" +
                "WPADRV=nl80211\n" +
...
//                "udhcpc -n -i %s -x hostname ${HOST_NAME} -p /var/run/udhcpc.wlan0.pid \n"
    ...
    public static final String shell_dnsmasq =
            "#!/bin/sh\n" +
            "\n" +
            "kill -9 $(ps aux | grep dnsmasq | grep -v grep | awk '{print $2}' ) || true\n" +
            "\n" +
            "# user variables\n" +
            "lan_if=wlan0\n" +
            "ip_stem=192.168.10\n" +
    ...
    private static final String shell_hostapd =
            ...
            "      kill -9 $(ps aux | grep hostapd | grep -v grep | awk '{print $2}' ) || true\n" +
            "      kill -9 $(ps aux | grep wpa_supplicant | grep -v grep | awk '{print $2}' ) || true\n" +
